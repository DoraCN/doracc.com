...
[节点](./node) 和 [算子](./operator) 作为构建块

[事件流](./event-stream) 作为其通信通道

[数据消息/Arrow Data](./data-message) 作为高效数据传输的格式。

现在，我们来谈谈如何实际编写这些节点和算子的代码 ，让它们完成各自的工作。你用 `Python` 或 `Rust` 编写的自定义逻辑如何连接到 `dora 运行时`来接收输入并发送输出？这时， **`API Binding` 绑定** 就派上用场了。
...
让我们看一下使用 `Python API` 绑定（ `DORA` 库）的简化示例，这是 `AI` 和数据处理任务的常见选择。
...
算子使用稍微不同的 `API` 结构，专门设计用于 `dora-runtime` 进程。
    ...
    def on_event(self, dora_event, send_output):
        ...
        if event_type == "INPUT":
             if event_id == "operator_input":
                ...
                send_output("operator_output", processed_data, dora_event["metadata"]) # 算子传递元数据
...
- `return DoraStatus.CONTINUE / DoraStatus.STOP`：算子在处理事件后明确向运行时返回状态，指示它们是否应该继续或停止。
...
`API 绑定`是必不可少的软件库，它使您能够使用熟悉的编程语言（例如 `Python`、`Rust`、`C` 和 `C++`）为自定义 `DORA` 节点和算子编写核心逻辑。它们提供了标准化的函数工具包（ `init` 、 `send_output` 、`事件循环/回调`），这些函数抽象了进程间通信、共享内存和事件处理的复杂性，使您可以专注于特定的应用程序任务，同时无缝集成到 `DORA` 数据流中。
