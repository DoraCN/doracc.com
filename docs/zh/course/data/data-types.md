...
上一节讲清了"为什么用 Arrow"。这一节我们动手：把小多最常打交道的**三类数据**——数字、数组、图像——一个个学会怎么**造**（发出去）、怎么**读**（收进来）。
...
朵拉魔盒的环境里已经装好了 NumPy，直接 `import numpy as np` 就能用。
...
图像是小多"眼睛"的核心数据，第七章会大量用到。这里先把"图像怎么在数据流里传"的原理和写法讲透。
