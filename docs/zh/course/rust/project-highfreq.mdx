...
本章学的东西，到这里派上用场。我们要做一件"名副其实"的事：用 **Rust** 写一个**高频运行**的信号处理节点（每秒跑上千次），再用 **Python** 把结果画出来。这是实战项目，用 Rust 为 DORA 实现 **高频加速**——让它的关键零件跑得飞快。
...
下一章 [第十章 · 让小莫动起来](../control/)，我们进入控制与仿真，让小莫在 PyBullet 里操纵机械臂。
