...
学完这节，第四章就圆满了，小多的 **✍️ 会表达** 能力就正式点亮。
...
:::tip 练习：做一个"心跳"节点
写一个节点，每隔可配置的秒数，就打印一句可配置的话（比如每 3 秒打印 "小多还活着~"）。要求秒数和那句话都来自 `env` 参数。
:::
...
def main():
    node = Node()
    message = os.getenv("MESSAGE", "小多还活着~")
    for event in node:
        if event["type"] == "INPUT":
...
下一章 [第五章 · 数据的语言：Arrow](../data/)，我们会把这一章反复出现的 `pa.array` 和 `.to_pylist()` 讲透，让小多学会处理各种各样的数据。
