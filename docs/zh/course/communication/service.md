...
上一节的 Topic 是"喊一嗓子、发完不管"。但很多时候，小多需要的是**一问一答**：问一句"现在几点？"，然后**等一个明确的答复**。这就是第二种模式——**Service（服务）请求应答**。
...
想象一个热闹的场景：小多**同时**问了服务端三个问题（"几点了？""剩多少电？""温度多少？"）。服务端陆续回了三个答复。问题来了——**小多怎么知道哪个答复对应哪个问题？**

如果答复只是"42""87""25"三个数字飞回来，小多完全对不上号！

**解决办法：给每个问题编一个号。** 小多问的时候，在问题上贴个标签"这是 1 号问题"；服务端回答时，也在答复上贴回"这是对 1 号问题的回答"。这样小多一看编号就对上了。
...
下一节学第三种模式 **Action（长任务）**——当小多要做一件**耗时**的事、还想**边做边汇报进度**时该怎么办。
