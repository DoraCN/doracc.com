---
description: 小项目③ 问答/任务交互——综合运用 Service 与 Action 两种通信模式，给小多做一个既能"一问一答"、又能"派发长任务并汇报进度"的交互中枢。
---

# 🎯 小项目③：问答 / 任务交互

本章学了四种通信模式，是时候把它们用起来了。这个小项目里，我们给小多做一个**交互中枢**：它既能回答简单问题（用 **Service**），又能接受耗时任务并汇报进度（用 **Action**）。

做完这个项目，小多就真正**掌握了"沟通"**——不再只会单向收发，而是能一问一答、能派活儿边做边报。
...
- **brain**：小多的交互中枢，收到提问立刻答复，收到任务就边做边报进度、最后给结果；
...
```python
# brain.py —— 小多的交互中枢：既答问题（Service），又做长任务（Action）
...
下一章 [第七章 · 让小多看见](../vision/)，我们给小多接上摄像头和物体检测，让它拥有"视觉"。
