...
从这一章开始，小多要解锁 **💬 沟通能力**。前面几章里，节点之间其实已经在"说话"了——只是我们没给这种说话方式起名字。这一章，我们把节点间的**四种对话方式**一次讲清。
...
小多的摄像头节点（`camera`）输出图像。现在"物体检测"和"录像存盘"两个节点都需要这份图像。请你写出 YAML 里这三个节点的 `inputs`/`outputs` 连线，并说明这用到了 Topic 的什么特性。
...
下一节，我们学第二种模式 **Service（请求应答）**——当小多需要"问一句、等一个答复"时该怎么办。
