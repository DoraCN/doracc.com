...
欢迎来到 `DORA` 教程的最后一章！在本教程中，我们学习了`数据流`、`节点`和`算子` ，以及它们如何通过事件流进行通信并高效地传输数`据消息/Arrow Data`。
...
至此，我们对 `DORA` 核心概念的入门教程就结束了。我们介绍了`数据流`、`构建块（节点、算子）`、`通信机制（事件流、数据消息/Arrow Data、API 绑定）`以及`运行时基础架构（命令行界面、守护进程、协调器）`。现在，您对 `DORA` 工作原理及其主要组件的作用有了基本的了解，这将帮助您更深入地构建自己的实时数据流应用程序。
