...
:::info
在早期的文档版本中，`Operator` 一词被翻译为`操作符`。然而，经过进一步的考虑和社区反馈，`算子`一词更符合其在数据处理和编程中的实际含义。`算子`通常用于描述执行特定计算或转换的独立模块，这与`DORA`中`Operator`的功能更为契合。因此，后续文档中将统一使用`算子`一词。
:::
...
现在，让我们探索一种在 `Node` 内部添加更多结构和可重用性的方法： **`Operator 算子`** 的概念。
...
可以将节点想象成工厂车间的工作站（一栋独立的建筑），而算子则是该工作站内的专用工具或机器。工作站（节点）是独立的单元，但其内部的工作由其工具集（算子）完成。

## 算子与自定义节点

让我们快速比较一下：

| 特征         | 自定义节点                                  | Dora 运行时节点和算子                                        |
| :----------- | :------------------------------------------ | :------------------------------------------------------------- |
| **运行方式** | 独立节点                                    | 单进程                                                         |
| **包含**     | 一段自定义的代码                            | 一个或多个算子                                               |
| **逻辑单元** | 整个节点代码                                | 单个算子                                                     |
| **通讯**     | 使用 `dora Node API` 直接与 `DORA` 系统交互 | 节点运行时使用 `dora Node API`; 算子使用 `dora Operator API` |
| **可重用性** | 重用整个 `Node` 进程                        | 重复使用单个算子逻辑模块                                     |
...
- 它有一个 `on_event` 方法（或函数），每当发生与该算子有关的事件（例如接收输入或停止信号）时， `dora-runtime` 就会调用该方法。
...
当您使用 `dora run` 和定义了带有算子的运行时节点的 `Dataflow YAML` 时，下面是发生情况的简化视图：
...
现在我们了解了节点和算子，让我们看看数据在系统中移动的基本概念： **`Event Stream 事件流`** 。
